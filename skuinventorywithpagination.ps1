...
function Get-CreationTime {
    ...
    if ($resource.createdTime) {
        try {
            ...
            Write-Output "Failed to parse top-level createdTime '$($resource.createdTime)' for $typeKey $resourceName`: $_"
            # Continue to next steps if parsing fails
        }
    }
    else {
        Write-Output "Top-level createdTime not available for $typeKey $resourceName"
    }
        ...
        try {
            ...
            foreach ($part in $path -split '\.') {
                if ($value.PSObject.Properties.Name -contains $part) {
                    ...
                    Write-Output "Property $part not found in $typeKey response for $resourceName"
                    break
                }
            }
            if ($null -ne $value) {
                try {
                    ...
                    Write-Output "Failed to parse creation time '$value' for $typeKey $resourceName`: $_"
                    # Continue to next steps if parsing fails
                }
            }
            else {
                Write-Output "Creation time is null for $typeKey $resourceName using path $path"
            }
        }
        catch {
            Write-Output "Error extracting creation time for $typeKey $resourceName using path $path`: $_"
        }
    }
        ...
        try {
            ...
            if ($null -ne $value) {
                try {
                    ...
                    Write-Output "Failed to parse creation time '$value' for $typeKey $resourceName using fallback path $fallbackPath`: $_"
                    continue
                }
            }
        }
        catch {
            Write-Output "Error extracting creation time for $typeKey $resourceName using fallback path $fallbackPath`: $_"
            continue
        }
    }

    # Step 4: If all attempts fail, return N/A
    Write-Output "No creation time found for $typeKey $resourceName"
    return "N/A"
}
...
$resList = @()
    ...
    do {
        try {
            $response = Invoke-RestMethod -Uri $resUri -Headers $headers -Method Get
            if ($response.value) {
                $resList += $response.value
            }
            $resUri = $response.nextLink
        } catch {
            Write-Output "⚠️ Error fetching resources: $_"
            break
        }
    } while ($resUri)
    ...
    foreach ($res in $resList) {
        ...
        $sku = try {
            ...
            Write-Output "⚠️ SKU error for $typeKey $($res.name): $_"
            "N/A"
        }
